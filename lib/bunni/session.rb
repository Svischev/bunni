...
module Bunni
  ...
  class Session
    ...
    # @option connection_string_or_opts [Integer] :continuation_timeout (15000) Timeout for client operations that expect a response (e.g. {Bunni::Queue#get}), in milliseconds.
    ...
    def initialize(connection_string_or_opts = ENV['RABBITMQ_URL'], optz = Hash.new)
      ...
      log_level        = opts[:log_level] || ENV["bunni_LOG_LEVEL"] || Logger::WARN
    ...
    def create_channel(n = nil, consumer_pool_size = 1, consumer_pool_abort_on_exception = false, consumer_pool_shutdown_timeout = 60)
      ...
      raise RuntimeError, "this connection is not open. Was Bunni::Session#start invoked? Is automatic recovery enabled?" if !connected?

      @channel_mutex.synchronize do
        if n && (ch = @channels[n])
          ch
        else
          ch = Bunni::Channel.new(self, n, ConsumerWorkPool.new(consumer_pool_size || 1, consumer_pool_abort_on_exception, consumer_pool_shutdown_timeout))
          ch.open
          ch
        end
      end
